...
| `check_interval` | int | `1` | **检测频率（分钟）**。<br>每隔多少分钟执行一次自动检测。建议不要设置过短以免被封禁 IP。 |
| `allowed_groups` | list | `[]` | **指令权限白名单**。<br>填写允许使用 `/steamstatus` 指令的群组统一标识符。<br>❌ 如果留空，则**所有群组**均可使用指令。<br>✅ 如果填写，则**只有列表中的群组**可以使用。 |
| `auto_push_groups` | list | `[]` | **自动推送目标名单**。<br>当 Steam 状态发生变更（如崩了或恢复了）时，会自动推送到这些群组。 |
